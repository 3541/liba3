...
#define A3_HT_DEFINE_METHODS_HASHER(K, V, H, C)                                \
    static uint64_t A3_HT_HASH(K, V)(A3_HT(K, V) * table, K key) {             \
        ...
        return ret ? ret : 1;                                                  \
                                                                               ...
                                                                               \
    static size_t A3_HT_PROBE_COUNT(K, V)(A3_HT(K, V) * table, size_t index,   \
                                          uint64_t hash) {                     \
        assert(table);                                                         \
        return (index + table->cap - hash % table->cap) % table->cap;          \
    }                                                                          \
                                                                               \
    static void A3_HT_INSERT_AT(K, V)(A3_HT(K, V) * table, uint64_t hash,      \
                                      K key, V value) {                        \
        assert(table);                                                         \
        assert(hash);                                                          \
        ...
        for (size_t i = hash % table->cap, probe_count = 0;;                   \
            ...
            if (!current_entry->hash) {                                        \
                ...
                return;                                                        \
                                                                               ...
                                                                               \
            if (A3_HT_PROBE_COUNT(K, V)(table, i, current_entry->hash) <       \
                probe_count) {                                                 \
    ...
    void A3_HT_RESIZE(K, V)(A3_HT(K, V) * table, size_t new_cap) {             \
        ...
        table->cap                      = new_cap;                             \
        table->entries                  = (A3_HT_ENTRY(K, V)*)(calloc(         \
            table->cap, sizeof(A3_HT_ENTRY(K, V))));          \
                                                                               \
    ...
    A3_SSIZE_T A3_HT_FIND_INDEX(K, V)(A3_HT(K, V) * table, K key) {            \
        ...
        for (size_t i = hash % table->cap, probe_count = 0;;                   \
            ...
            A3_HT_ENTRY(K, V)* current_entry = &table->entries[i];             \
            if (!current_entry->hash ||                                        \
                A3_HT_PROBE_COUNT(K, V)(table, i, current_entry->hash) <       \
                    probe_count)                                               \
                return -1;                                                     \
    ...
    void A3_HT_INIT(K, V)(A3_HT(K, V) * table, bool can_grow) {                \
        ...
        for (size_t i = 0;                                                     \
             i < A3_HT_HASH_KEY_SIZE * sizeof(table->hash_key[0]); i++) {      \
            key_bytes[i] = (uint8_t)rand();                                    \
        }                                                                      \
        table->entries =                                                       \
            (A3_HT_ENTRY(K, V)*)calloc(table->cap, sizeof(A3_HT_ENTRY(K, V))); \
    ...
    bool A3_HT_DELETE_INDEX(K, V)(A3_HT(K, V) * table, size_t index) {         \
        ...
        while (next_entry->hash &&                                             \
               A3_HT_PROBE_COUNT(K, V)(table, i, next_entry->hash)) {          \
            entry->hash = 0;                                                   \
            *entry      = *next_entry;                                         \
            i           = (i + 1) % table->cap;                                \
            entry       = next_entry;                                          \
