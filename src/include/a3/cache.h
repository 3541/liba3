...
#define A3_CACHE_DECLARE_METHODS(K, V)                                         \
    A3_H_BEGIN                                                                 \
                                                                               \
    void A3_CACHE_INIT(K, V)(A3_CACHE(K, V)*, size_t capacity,                 \
                             A3_CACHE_EVICT_CB(K, V));                         \
    A3_CACHE(K, V) *                                                           \
        A3_CACHE_NEW(K, V)(size_t capacity, A3_CACHE_EVICT_CB(K, V));          \
...
#define A3_CACHE_DEFINE_METHODS_NOHT(K, V)                                     \
    void A3_CACHE_INIT(K, V)(A3_CACHE(K, V) * cache, size_t capacity,          \
        ...
        A3_UNWRAPN(cache->accessed,                                            \
                   (size_t*)calloc(capacity / A3_CACHE_ENTRIES_PER_BLOCK,      \
                                   sizeof(size_t)));                           \
        A3_HT_INIT(K, V)(&cache->table, false);                                \
        A3_HT_RESIZE(K, V)(&cache->table, capacity);                           \
    }                                                                          \
                                                                               \
    A3_CACHE(K, V) * A3_CACHE_NEW(K, V)(size_t capacity,                       \
                                        A3_CACHE_EVICT_CB(K, V)                \
                                            eviction_callback) {               \
        assert(capacity > 0);                                                  \
                                                                               \
        A3_CACHE(K, V)* ret =                                                  \
            (A3_CACHE(K, V)*)calloc(1, sizeof(A3_CACHE(K, V)));                \
        if (!ret)                                                              \
            return NULL;                                                       \
    ...
    static void A3_CACHE_ACCESS(K, V)(A3_CACHE(K, V) * cache, size_t index) {  \
        ...
        *block |= bit;                                                         \
                                                                               ...
                                                                               \
    static bool A3_CACHE_ACCESSED(K, V)(A3_CACHE(K, V) * cache,                \
                                        size_t index) {                        \
        assert(cache);                                                         \
    ...
    V* A3_CACHE_FIND(K, V)(A3_CACHE(K, V) * cache, K key) {                    \
        ...
        return &cache->table.entries[i].value;                                 \
                                                                               ...
                                                                               \
    static void A3_CACHE_EVICT(K, V)(A3_CACHE(K, V) * cache,                   \
                                     void* callback_ctx) {                     \
        assert(cache);                                                         \
                                                                               \
        ...
        memset(cache->accessed, 0,                                             \
               cache->table.cap / A3_CACHE_ENTRIES_PER_BLOCK *                 \
                   sizeof(size_t));                                            \
    }                                                                          \
                                                                               \
    void A3_CACHE_INSERT(K, V)(A3_CACHE(K, V) * cache, K key, V value,         \
                               void* callback_ctx) {                           \
        assert(cache);                                                         \
                                                                               \
